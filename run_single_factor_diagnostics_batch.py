"""Compatibility wrapper for the Rust single-factor diagnostics batch runner."""
...
def build_delegated_command(argv: list[str] | None = None) -> list[str]:
    return [*_rust_binary_command(), "single-factor-batch", *(argv if argv is not None else sys.argv[1:])]
