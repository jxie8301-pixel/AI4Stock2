...
AkShare Python 代码现在只作为 PyO3 provider adapter 保留；正式入口是 Rust collector。

如果你要切到 `proxy_patch` 后端，需要显式传入：
```bash
cargo run --bin ai4stock-collect -- akshare --update --network-backend proxy_patch --proxy-auth-token <TOKEN> --workers 8
```
...
```bash
cargo run --bin ai4stock-collect -- akshare --update --refresh-stock-list --workers 8
```
...
```bash
cargo run --bin ai4stock-collect -- akshare --refresh-stock-list-only
```
...
```bash
cargo run --bin ai4stock-collect -- akshare --rebuild-processed --workers 8
```
...
刷新 Tushare 股票列表缓存：
```bash
cargo run --bin ai4stock-collect -- tushare --refresh-symbols-only
```

抓取全量或缓存股票池：
```bash
cargo run --bin ai4stock-collect -- tushare --all --workers 8 --end-date 2026-03-31
```

按本地已有 symbol + 缓存股票池做增量更新：
```bash
cargo run --bin ai4stock-collect -- tushare --update --workers 8 --end-date 2026-03-31
```

只刷新 Tushare 指数 benchmark 文件：
```bash
cargo run --bin ai4stock-collect -- tushare --refresh-benchmarks-only --end-date 2026-03-31
```

如果你希望在更新股票 raw 的同时顺手刷新 benchmark：
```bash
cargo run --bin ai4stock-collect -- tushare --update --refresh-benchmarks --workers 8 --end-date 2026-03-31
```

只跑少量 symbol 做验证：
```bash
cargo run --bin ai4stock-collect -- tushare --symbols 600000,000333 --workers 2 --end-date 2026-03-31
```

如果 raw 已经完整，只想从本地 raw 重建 `hfq` 规范化 parquet：
```bash
cargo run --bin ai4stock-collect -- tushare --rebuild-processed --workers 8
```
...
cargo run --bin ai4stock-gen-feature -- generate \
  ...
  --workers 8
```
生产入口统一使用 Rust binary；旧 Python feature generator 已移除。
...
Rust feature generation 当前按 bucket 生成完整输出；后续如需增量语义，应在 Rust 入口显式实现，不再回退到 Python runtime。
...
`ai4stock-gen-feature` 现在只负责生成统一全集 cache，不再暴露 `alpha158/alpha360` 这类历史因子库名字。
...
```bash
cargo run --bin ai4stock-experiment -- batch --pipeline rolling --experiment-profile core_v4_lgbm_default_10x20x10 --sweep 'rolling.retrain_step=[5,10,15]' --dry-run
```
...
改 feature profile、model profile、experiment profile，都不需要重新执行 `ai4stock-gen-feature`。
只有当 unified factor store 的生成空间本身变化时，才需要重建 cache。

为什么 `ai4stock-gen-feature` 仍然独立存在，而不是在主训练脚本里隐式生成：
