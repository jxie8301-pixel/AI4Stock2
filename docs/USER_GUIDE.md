...
如果要切到 Tushare 数据源：
```bash
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --data-source tushare
```
...
```bash
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --feature-profile alpha158_full --run-tag alpha158_full
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --model-profile lgbm_fast --run-tag fast_profile
```

同一实验做策略参数对比时，可以在 experiment profile 基础上直接覆写：
```bash
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --topk 20 --n-drop 4 --run-tag top20_drop4
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --topk 30 --n-drop 5 --run-tag top30_drop5
```

如果你不想为了一个小参数改动再复制一份 experiment yaml，现在也可以直接用通用覆写：
```bash
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --set strategy.topk=20 --set rolling.retrain_step=5
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --set label.signal_horizon=10
```
...
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --load-models
```
...
如需关闭：
```bash
pixi run python run_native_rolling.py --experiment-profile core_v4_lgbm_default_10x20x10 --disable-local-store
```
