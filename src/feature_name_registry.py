...
def get_tushare_factor_feature_names(config: dict[str, Any] | None = None) -> list[str]:
    ...
    names += [
        ...
        "fi_margin_quality",
        "fi_profitability_combo",
        "fi_growth_quality_combo",
        "fc_p_change_mid",
        "fc_p_change_width",
        "fc_net_profit_mid",
        "fc_net_profit_width",
        "fc_net_profit_mid_ratio",
        "fc_positive_confidence",
        "fc_days_since_ann",
        "fc_freshness_weight",
        "fc_surprise_fresh",
        "exp_growth_combo",
        "exp_profit_quality_gap",
        "exp_asset_efficiency",
        "exp_profit_margin_proxy",
        "exp_op_margin_proxy",
        "exp_days_since_ann",
        "exp_freshness_weight",
        "exp_growth_fresh",
        f"up_down_turnover_ratio_{long_flow_window}",
        f"return_turnover_corr_{long_flow_window}",
        f"downside_turnover_pressure_{long_flow_window}",
        "days_since_last_up_limit",
        "days_since_last_down_limit",
        "up_limit_streak",
        "down_limit_streak",
        f"near_up_limit_count_{long_flow_window}",
        f"near_down_limit_count_{long_flow_window}",
        f"industry_breadth_accel_{int(industry_windows[-2])}_{int(industry_windows[-1])}",
        f"industry_momentum_accel_{int(industry_windows[-2])}_{int(industry_windows[-1])}",
        f"stock_industry_ret_gap_{int(industry_windows[0])}_{int(industry_windows[-2])}",
        f"stock_industry_ret_gap_{int(industry_windows[-2])}_{int(industry_windows[-1])}",
