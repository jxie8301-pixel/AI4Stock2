...
def _rust_binary_command() -> list[str]:
    env_value = os.environ.get("AI4STOCK_EXPERIMENT_BIN")
    if env_value:
        return shlex.split(env_value)
    return ["cargo", "run", "--bin", "ai4stock-experiment", "--"]


def build_delegated_command(argv: list[str] | None = None) -> list[str]:
    return [*_rust_binary_command(), "batch", *(argv if argv is not None else sys.argv[1:])]
