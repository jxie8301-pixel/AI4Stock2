...
- [x] Move LightGBM bundle runtime ownership to Rust while keeping Python only for training:
  ...
  - Rust commands that embed Python should be launched as `pixi run cargo ...`; PyO3 then uses the pixi Python from the active environment, and Rust prepares `sys.path` from that interpreter's own `sysconfig` paths instead of relying on `.cargo/config.toml`
